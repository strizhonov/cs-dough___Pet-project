...
CREATE TABLE IF NOT EXISTS `cs_dough_db`.`user_account` (
  ...
  `user_type` ENUM('ANONYMOUS', 'USER', 'ADMIN') NULL DEFAULT 'ANONYMOUS',
  `language` ENUM('EN', 'RU') NULL DEFAULT 'EN',
...
CREATE TABLE IF NOT EXISTS `cs_dough_db`.`tournament` (
  ...
  `tournament_status` ENUM('UPCOMING', 'ONGOING', 'FINISHED') NOT NULL DEFAULT 'UPCOMING',
