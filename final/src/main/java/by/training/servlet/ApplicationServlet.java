...
import by.training.appentry.SingleBeanApplicationContext;
...
public class ApplicationServlet extends HttpServlet {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ActionCommandProvider commandProvider
                = (ActionCommandProvider) SingleBeanApplicationContext.getInstance().get(ActionCommandProviderImpl.class);
        ActionCommand command = commandProvider.get(request);

        SecurityService securityService
                = (SecurityService) SingleBeanApplicationContext.getInstance().get(SecurityService.class);

        if (!securityService.canExecute(command)) {
            ServletRouter router = securityService.route(command, request.getAttribute(ApplicationConstantContainer.USER));
